...
arguments_strFirst = './spynet_files/first.png'
arguments_strSecond = './spynet_files/second.png'
...
class Network(torch.nn.Module):
    def __init__(self):
        ...
        self.load_state_dict({ strKey.replace('module', 'net'): tenWeight for strKey, tenWeight in torch.hub.load_state_dict_from_url(url='./spynet_files/network-sintel-final.pytorch', file_name='spynet-' + arguments_strModel).items() })
    # end
...
if __name__ == '__main__':
  im0 = PIL.Image.open('spynet_files/00001_I0.png')
  im0 = im0.resize((1024, 416))

  im1 = PIL.Image.open('spynet_files/00001_I1.png')
  im1 = im1.resize((1024, 416))

  im2 = PIL.Image.open('spynet_files/00001_I2.png')
  im2 = im2.resize((1024, 416))

  im3 = PIL.Image.open('spynet_files/00001_I3.png')
  im3 = im3.resize((1024, 416))

  im4 = PIL.Image.open('spynet_files/00001_I4.png')
  im4 = im4.resize((1024, 416))

  images = [im0, im1, im2, im3, im4]

  for i, pair in enumerate(itertools.combinations(images, 2)):
    print(i, pair)
    first = torch.FloatTensor(numpy.ascontiguousarray(numpy.array(pair[0])[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0)))
    second = torch.FloatTensor(numpy.ascontiguousarray(numpy.array(pair[1])[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0)))

    output = estimate(first, second)

    objOutput = open('spynet_files/out_{}.flo'.format(i), 'wb')

    numpy.array([ 80, 73, 69, 72 ], numpy.uint8).tofile(objOutput)
    numpy.array([ output.shape[2], output.shape[1] ], numpy.int32).tofile(objOutput)
    numpy.array(output.numpy().transpose(1, 2, 0), numpy.float32).tofile(objOutput)
